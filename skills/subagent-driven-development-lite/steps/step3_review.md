...
## Actions
1. **Prepare Review Briefing:** Use the `run_command` action to run the preparation script to generate the diff and review briefing:
   `node .agents/skills/subagent-driven-development-lite/scripts/prepare-brief.js reviewer HEAD docs/superpowers/plans/PLAN_FILE_NAME.md N`
...
3. **Start Waiting:** Once dispatched:
   * Action: Use the `wait` action for 60 seconds.
